...
def delete_doc(del_doc_number):
    doc_exist = check_document_existence(del_doc_number)
    if doc_exist:
        for current_document in documents:
            doc_number = current_document['number']
            if doc_number == del_doc_number:
                documents.remove(current_document)
                res = remove_doc_from_shelf(doc_number)
                return  True
    else:
        return False
...
def move_doc_to_shelf(user_doc_number, user_shelf_number):
    ...
    res = remove_doc_from_shelf(user_doc_number)
    append_doc_to_shelf(user_doc_number, user_shelf_number)
    return f'Документ номер "{user_doc_number}" был перемещен на полку номер "{user_shelf_number}"', True
...
def show_all_docs_info():
    ...
    for current_document in documents:
        print(show_document_info(current_document))
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            directory_number = get_doc_shelf(doc_num)
            if directory_number:
            ...
            new_doc_owner_name = input('Введите имя владельца документа- ')
            new_doc_shelf_number = input('Введите номер полки для хранения - ')
            res = add_new_doc(new_doc_number, new_doc_type, new_doc_owner_name, new_doc_shelf_number)
            if res:
                print(f'\nСоздан документ {new_doc_number} и добавлен на полку {new_doc_shelf_number}')
        elif user_command == 'd':
            del_doc = input('Введите номер документа - ')
            doc_number, deleted = delete_doc(del_doc)
            if deleted:
                print(f'Документ с номером "{del_doc}" был успешно удален')
        elif user_command == 'm':
            user_doc_number = input('Введите номер документа - ')
            user_shelf_number = input('Введите номер полки для перемещения - ')
            ret, move = move_doc_to_shelf(user_doc_number, user_shelf_number)
            print(ret)
        elif user_command == 'as':
            shelf_number, added = add_new_shelf()
            if added:
                print(f'Добавлена полка {shelf_number}')
